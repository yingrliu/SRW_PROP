...
class OptimParticleSwarm(_optimizer):
    ...
    def init_particles(self, velocity_range):
        """
        ...
        self.particles = np.asarray([scale[0] for pos, scale in zip(self.tunable_params_positions, self.tunable_params_ranges)]
                                    ).reshape((1, -1)).repeat(len(self.tunable_params_positions), axis=0)
        #
        ...
        #
        self.local_best_rewards = np.ones(shape=(len(self.tunable_params_positions),)) * self.global_best_reward
        ...
        self.single_step(1.0, 0.0, 0.0, 1.0)
        return
    ...
    def forward(self, velocity_range, inertia_coeff, cognitive_coeff, social_coeff, step_size,
                num_steps=100, early_stopping=1, saveto=None):
        self.init_particles(velocity_range)
